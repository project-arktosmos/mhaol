use mhaol_backend::{api, load_env, AppState};
...
async fn main() {
    ...
    #[cfg(not(target_os = "android"))]
    {
        ...
        tokio::spawn(async move {
            mhaol_backend::llm_worker::run_llm_worker(llm_state).await;
        });
    }
    ...
    #[cfg(not(target_os = "android"))]
    {
        ...
        tokio::spawn(async move {
            match mhaol_backend::peer_service::PeerServiceManager::new(peer_state) {
                Ok(mut manager) => {
