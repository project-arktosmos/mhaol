<script lang="ts">
  ...
  setContext('browseViewMode', {
    get value() { return browseViewModeValue; }
  });
...
<div class="flex h-screen flex-col">
  <Navbar brand={{ label: "Mhaol", highlight: "Flix" }} classes="!bg-base-300">
    {#snippet end()}
      <button
        class="btn btn-circle btn-ghost btn-sm"
        onclick={() => (browseViewModeValue = browseViewModeValue === 'poster' ? 'backdrop' : 'poster')}
        aria-label="Toggle view mode"
        title={browseViewModeValue === 'poster' ? 'Switch to backdrop view' : 'Switch to poster view'}
      >
