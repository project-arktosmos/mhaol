...
pub async fn ws_handler(
    ...
    // Extract name and instance_type from passport if provided
...
fn extract_passport_info(
    ...
    // Verify passport signature matches the peer address
    ...
    // Parse passport JSON for name and instanceType
...
async fn handle_socket(
    ...
    // Generate ICE servers for this connection
    let ice_servers = turn::generate_credentials(&state.config);

    // Send connected message with ICE servers
    ...
    // Send room-peers with full peer info
...
fn handle_message(state: &WsState, room_id: &str, from_peer_id: &str, text: &str) {
...
fn cleanup(state: &WsState, room_id: &str, connection_id: &str) {
