...
const PORT = parseInt(envPort || '1410');
const BACKEND_PORT = PORT + 1; // internal only, not exposed
const HEALTH_URL = `http://localhost:${BACKEND_PORT}/api/identities`;
...
if (skipBuild && existsSync(join(STATIC_DIR, 'index.html'))) {
	...
	execSync('pnpm --filter identity build', { stdio: 'inherit', cwd: '../..' });
}
...
server.listen(PORT, '0.0.0.0', () => {
	...
	console.log(`Identity app running on:`);
