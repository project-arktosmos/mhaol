use crate::documents::{Artist, FileEntry, ImageMeta, SubsLyrics, SyncedLine};
...
async fn search_lrclib(query: &str) -> Result<Vec<SubsLyrics>, (StatusCode, Json<serde_json::Value>)> {
    let url = format!("{}/search?q={}", LRCLIB_BASE, urlencoding(query));
    let res = reqwest::Client::new()
        .get(&url)
        .header("Accept", "application/json")
        .header("User-Agent", "Mhaol/1.0 (https://github.com/project-arktosmos/mhaol)")
        .send()
        .await
