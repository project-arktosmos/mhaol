...
//! `playerService.playUrl()` (yt-dlp direct streams). That path doesn't need
//! a stream-status flag or a `playable` list, but the service's initializer
//! hits both endpoints unconditionally — these stubs let it settle without
//! the user seeing an error toast.
