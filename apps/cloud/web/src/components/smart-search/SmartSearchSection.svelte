<script lang="ts">
	...
	import SmartSearchResultsTable from '$components/smart-search/SmartSearchResultsTable.svelte';
	import { scoreResults, findBestCandidate } from '$utils/smart-search/score';
	...
	let mediaConfig = $derived.by(() => {
		...
		const key = selection.type === 'movie' ? 'movies' : selection.type === 'tv' ? 'tv' : 'music';
