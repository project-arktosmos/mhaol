...
├── media_trackers.rs    # /api/media-trackers — per-(firkin, track?, user) playback time totals. `POST /heartbeat` upserts a `media_tracker` row keyed `sha256(firkin_id:address[:track_id])` and adds the supplied `deltaSeconds`; the right-side player calls it once at play-start (delta 0) and every 10 seconds while streaming. When the navbar music playlist surfaces a per-track context (`trackId` from MusicBrainz), the heartbeat buckets time per song; otherwise the row aggregates at the firkin level.
...
├── tmdb_match.rs        # Per-file TMDB match used at scan time for `local-movie` libraries. Parses (title, year) out of a video file's relative path, hits TMDB `/search/movie`, and scores candidates by token overlap + year proximity + original-title overlap + vote_average bias. cfg(not(target_os = "android")).
...
- `track-resolver.svelte.ts` — `TrackResolver` class. Pure projection — *no in-browser searches anywhere*. Single entry point `loadFromFirkin({ releaseGroupId, files })` fetches the MusicBrainz tracklist and pairs each track with its persisted YouTube URL + lyrics from the firkin's `files`, returning `{ missingAny }`. The detail page uses `missingAny` to decide whether to poll for the rolled-forward firkin while the server's background album resolver runs (see `POST /api/firkins` below — fresh musicbrainz albums kick off `spawn_resolve_album_tracks` automatically). The virtual catalog page also uses `loadFromFirkin({ files: [] })` to surface the tracklist for preview without ever searching, so navigating away never interrupts anything. Lyrics persisted on the firkin live as `'lyrics'`-typed `FileEntry` rows whose `value` is the JSON `{ source, externalId, syncedLyrics, plainLyrics, instrumental }`; the resolver decodes this on read and parses the LRC text into the existing `SubsLyricsItem` shape so `CatalogTracksCard` can show synced/plain/instrumental status without re-fetching.
...
`apps/cloud/web/src/routes/+layout.svelte` is navbar + main only; there is no right-side aside. The only persistent overlay is the fixed bottom-right `NavbarAudioPlayer` (with `NavbarLyricsPanel` and `NavbarPlaylistPanel`), shown when `playerService.displayMode === 'navbar'` and a file is loaded. The layout calls `playerService.initialize()` on mount so the player's stores wake up; the `/api/player/stream-status` and `/api/player/playable` stubs let initialize settle without errors.

Audio playback uses the dedicated `displayMode === 'navbar'` mode. `NavbarAudioPlayer.svelte` is a compact horizontal strip (thumbnail, title, play/pause, position, seek bar, duration, stop) that owns its own hidden `<video>` element wired to `playerService.state.directStreamUrl`. The element is kept mounted at all times so its attach effect can drive `src` / `play()` whenever the reactive condition flips back to "navbar mode active". Audio callers (the catalog tracks card via `playYouTubeAudio` in `apps/cloud/web/src/lib/youtube-match.service.ts`, and the `/youtube` page when `mode === 'audio'`) call `playerService.playUrl(file, url, mime, 'navbar')` to surface playback here. Firkin in-page playback (`/catalog/[ipfsHash]`) uses `'inline'` as before; the legacy `'sidebar'` / `'fullscreen'` video display modes no longer have a host surface in the cloud layout.
...
- `GET /api/libraries/:id/scan` — recursively walk the library directory and return `{ root, total_files, total_size, entries }` where each entry is `{ path, relative_path, size, mime, tmdbMatch? }`. MIME types are resolved by extension via `mime_guess`. The library's `last_scanned_at` is updated once the walk completes, and the scan response itself is persisted to the `library_scan_result` table (one row per library, replaced on every scan) so the WebUI can re-display the file list on page reload without having to re-scan. When the library's `addons` includes `local-movie`, every video entry runs through `tmdb_match::match_movies_parallel` (bounded concurrency 5) before the response is built: the relative path is parsed into `(title, year)` via `extract_movie_query` (handles both the canonical `Movie Name (2023)/movie.mkv` form and scene-release filenames like `Inception.2010.1080p.BluRay.x264-GROUP.mkv`), TMDB's `/search/movie` is hit (year-filtered first, then a no-year fallback), and candidates are scored by token overlap (≥50% gate, ×10) + year proximity (+6 exact, +3 ±1y, +1 ±2y) + original-title overlap (×2) + a small `vote_average` bias (×0.1). The best candidate is attached as `tmdbMatch = { tmdbId, title, originalTitle?, year?, overview?, posterUrl?, voteAverage?, score }` so the WebUI's scan-results table can show "this video file matched to <TMDB title> (year)" inline. Matching is best-effort — if `TMDB_API_KEY` is unset or the upstream is unreachable, entries simply lack `tmdbMatch` and the rest of the scan flow is unaffected. After the walk, the scan handler hands off to `library_scan::schedule_pins` (background task that pins every matched file to IPFS so the WebUI can show a CID per file). The pin task waits for the IPFS node to reach `Running` state (up to ~60s) before it starts so the very first scan after server boot doesn't race the IPFS init. **No firkin records are created from a library scan** — the firkin store is for explicit bookmarks and catalog flows only.
- `GET /api/libraries/:id/scan-result` — returns the most recent persisted scan response from the `library_scan_result` table as `{ library_id, root, total_files, total_size, entries, scanned_at }`. `404` if the library has never been scanned. The `library_scan_result` row is also deleted when its parent library is removed via `DELETE /api/libraries/:id`.
...
- `GET /api/firkins` — list firkins persisted in SurrealDB (`firkin` table). Results are passed through `collapse_to_chain_heads`: (1) superseded versions (any id appearing in another row's `version_hashes`) are dropped so callers only see the head of each chain; (2) parallel chains with the same `(addon, title, year)` are collapsed to the head with the higher `version` (ties broken by `updated_at`). This handles the common case where the same logical item exists as both a "Bookmark" chain (no files, started at v=0) and a library-scan chain (files attached, may have rolled forward). The same dedup is applied to `GET /api/libraries/:id/firkins`. Each row carries `artistIds` (CIDs of the referenced `artist` records, drives the firkin's own CID) and the resolved `artists` (server-side join — `[{ id, name, roles: string[], imageUrl?, … }]`, where `roles` is the deduped multi-role array on the canonical artist record).
...
- `POST /api/firkins` — for fresh `musicbrainz` albums, the create handler spawns `spawn_resolve_album_tracks(state, id)` as a fire-and-forget `tokio::spawn` task immediately after the firkin lands in SurrealDB. The HTTP response returns the un-resolved firkin right away; the background task fetches the MB tracklist, runs all per-track YouTube + LRCLIB lookups, and rolls the firkin forward server-side. The browser never participates, so closing the tab / navigating to `/catalog/[ipfsHash]` / leaving the detail page does not interrupt anything — the resolution's lifetime is bound to the spawned future, not the request. The detail page uses `firkin.id`-keyed polling (every 4s while `loadFromFirkin` reports any track missing) to detect the rolled-forward CID and `goto` to it.
- `POST /api/firkins/:id/resolve-tracks` — explicit / manual variant of the same server-side per-track resolver for `musicbrainz` firkins (also used as the inner helper for the auto-spawn above; safe to re-trigger because the loop skips tracks that already have both YT URL and lyrics in `files`).
- `GET /api/firkins/:id/resolution-progress` — live per-track progress for an in-flight album resolution. Returns `{ firkinId, started_at, updated_at, completed, completedId?, tracks: [{ position, title, lengthMs?, youtubeStatus, youtubeUrl?, lyricsStatus, lyrics? }] }`. Backed by `state.track_progress` (an in-memory `Arc<RwLock<HashMap<firkin_id, AlbumProgress>>>` defined in `apps/cloud/src/track_progress.rs`). The album resolver writes to it as each track flips `pending` → `searching` → `found` / `missing`, and the WebUI polls this endpoint every 1s while the firkin still has no `lyrics`-typed `files`. `lyrics` carries the raw LRCLIB body (`{ source, externalId, syncedLyrics, plainLyrics, instrumental }`) so the inline lyrics preview works the moment a track resolves — well before the firkin itself rolls forward. `404` once the entry has been cleaned up (or never existed). Server restart loses progress; the resolver is idempotent so a re-trigger picks up where it left off. Loads the firkin, extracts the MusicBrainz release-group id from its `files` (the `https://musicbrainz.org/release-group/<id>` URL the bookmark / catalog flow seeds), fetches the album's tracklist from `https://musicbrainz.org/ws/2/release?release-group=<id>&inc=recordings`, then for each track that's still missing a YouTube URL or lyrics entry runs both lookups in parallel: a YouTube InnerTube search via `mhaol_yt_dlp::search::search_query` and an LRCLIB free-text search via `crate::search::lrclib_search_raw`. Both results go through token-based scorers (`track_resolve::pick_best_youtube_match` and `pick_best_lyrics_match` — Rust mirrors of the WebUI's `pickBestYouTubeMatch` / `pickBestLyricsMatch` with the same ≥50% title-token gate, artist/album hits, and duration-delta bonuses). Each resolved track adds a `url`-typed FileEntry (YouTube watch URL, `title = track title`) and a `lyrics`-typed FileEntry whose `value` is the JSON `{ source: "lrclib", externalId, syncedLyrics, plainLyrics, instrumental }`. Already-resolved tracks (matched by `(kind, title)` in `files`) are skipped, so re-running is idempotent. After all tracks are walked, the firkin is rolled forward via `rollforward_firkin` (same path as `enrich`/torrent-completion: push old id onto `version_hashes`, increment `version`, recompute CID, delete old record, create new at new CID, re-pin body to IPFS). Returns the new `FirkinDto`. The detail page (`/catalog/[ipfsHash]`) calls this once per visit when `loadFromFirkin` reports any missing track data; it then navigates to the new content-addressed id. Non-musicbrainz firkins return `400`.
...
- **Firkin `reviews`**: firkin records carry an optional `reviews: [{ label, score, maxScore, voteCount? }]` array — one entry per upstream rating source. TMDB contributes `{ label: "TMDB", score: vote_average, maxScore: 10, voteCount: vote_count }` and MusicBrainz contributes `{ label: "MusicBrainz", score: rating.value, maxScore: 5, voteCount: rating.votes-count }`. Both are extracted server-side by `GET /api/catalog/:addon/:id/metadata` (TMDB reads them off the same `append_to_response=credits,videos` detail response that supplies artists + trailers; MusicBrainz adds `inc=ratings` to the artist-credit fetch). The `/catalog/virtual` page captures the array on bookmark and `POST /api/firkins` bakes it into the CID; the `/catalog/[ipfsHash]` page back-fills missing reviews on first visit by hitting `/metadata` and `PUT`-ing them, which rolls the firkin forward to a new CID. The field participates in `compute_firkin_cid` (so a new score means a new version) but `reviews` is `skip_serializing_if = "is_empty"` and `voteCount` is `skip_serializing_if = "Option::is_none"` in the canonical body view, so existing firkin CIDs stay stable.
...
- `GET /api/media-trackers?firkinId=<>&trackId=<>&address=<>` — list rows from the `media_tracker` table (one row per (firkin, track?, user) tuple). Each row is `{ id, firkinId, trackId?, trackTitle?, address, totalSeconds, last_played_at, created_at, updated_at }`, sorted by `last_played_at` descending. All three query params are optional; `address` is normalised to lowercase 0x-prefixed hex before the match. Pre-per-track rows persist with `trackId`/`trackTitle` absent and aggregate time at the firkin level — the WebUI's `/consumption` page sums them alongside per-track rows under the same firkin so totals stay correct across the schema change.
...
- `POST /api/media-trackers/heartbeat` — body `{ firkinId, trackId?, trackTitle?, address, deltaSeconds }`. Upserts the tracker row keyed by `sha256(firkinId:address[:trackId])`, adds `deltaSeconds` to `totalSeconds`, and stamps `last_played_at`. `deltaSeconds` must be a finite non-negative number (`0` is allowed and is what the right-side player sends as the play-start signal). When `trackId` is supplied (set by the navbar music playlist with the MusicBrainz track id and forwarded via `playerService.state.trackId`), the row is bucketed against the (firkin, track, user) tuple instead of the firkin row so per-song listening time accrues separately within an album. `trackTitle` is captured opportunistically on first heartbeat for the row so the consumption UI can label the row without re-resolving the album. The cloud WebUI calls this from `media-tracker.service.ts` once when a firkin file starts playing in the right-side `PlayerVideo` (or audio in the navbar player) and again every 10 seconds while the player is `connectionState === 'streaming'` and not paused; flipping `trackId` mid-album restarts the heartbeat loop so swaps don't bleed time into the wrong row. Returns the persisted row. `404` if the firkin doesn't exist; `400` for an invalid address or a negative/NaN delta.
...
- `GET /api/catalog/:addon/:id/metadata` — returns `{ artists: [{ name, role?, imageUrl? }], trailers: [{ youtubeUrl, label?, language? }], reviews: [{ label, score, maxScore, voteCount? }] }` for a single upstream catalog item. `reviews` carries the upstream's user-rating snapshot (TMDB `vote_average`/`vote_count` out of 10; MusicBrainz `rating.value`/`votes-count` out of 5). For TMDB the score is read off the same detail response used for credits + videos so no extra round-trip is needed; for MusicBrainz `inc=artist-credits+ratings` collapses ratings into the artist-credit fetch. Reviews with zero votes are filtered out so a fresh / niche item doesn't carry a hollow `0.0 / 10 (0 votes)` entry. For TMDB (`tmdb-movie` / `tmdb-tv`) this collapses to one upstream HTTP call via `append_to_response=credits,videos` — the `credits` block becomes `artists` (top cast + selected crew jobs), and the `videos` block is filtered to YouTube `Trailer` entries whose `iso_639_1 == "en"` (English-only; non-English trailers are dropped here so the WebUI surfaces only English trailers — when none survive, the frontend falls back to the YouTube fuzzy search). Official trailers sort first; `language` carries TMDB's `iso_639_1` (lower-cased) so persisted firkins remember the language of each trailer. For `musicbrainz` / `youtube-video`, `artists` is populated from the upstream provider and `trailers` is `[]`. Used by the `/catalog/virtual` page on bookmark and the `/catalog/[ipfsHash]` detail page to backfill missing `artists` and to seed `trailers` (so the YouTube fuzzy-search fallback only runs when TMDB has no English trailer for the item / season).
...
`apps/cloud/src/library_scan.rs` runs after every `/api/libraries/:id/scan`. Its only side-effect is pinning files to the embedded IPFS node so the WebUI's libraries table can show a CID per file:
