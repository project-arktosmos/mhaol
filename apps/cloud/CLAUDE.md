...
├── recommendations.rs   # /api/recommendations — per-(user, recommended firkin) counts + watched/score annotations. `POST /ingest` dedupes per (user, source firkin) via the `recommendation_source` marker table so revisits don't re-count; only the `/catalog/[ipfsHash]` detail page (never `/catalog/virtual`) calls ingest. `compute_recommendation_cid` produces a stable CID for each related item using the same canonical body as a virtual firkin with empty artists/trailers. Each row also persists `upstream_id` (TMDB / MusicBrainz id) so `cleanup_for_firkin` — invoked from `firkins::create` after `pin_firkin_body` — can drop matching rows once the user adds the firkin to their collection (matches by `(address, addon, upstream_id)`, which survives the CID divergence between the virtual recommendation and the bookmarked firkin).
...
- `GET /api/recommendations?address=<>` — list rows from the `recommendation` table for the given user (one row per (user, recommended firkin) pair). Each row is `{ id, address, firkinId, addon, upstreamId, title, year?, description?, posterUrl?, backdropUrl?, count, watched, score, created_at, updated_at }`, sorted by `count` descending then `updated_at` descending. `address` is required and normalised to lowercase 0x-prefixed hex before the match. **Auto-cleanup**: when a firkin is created via `POST /api/firkins`, every recommendation row matching `(creator address, firkin.addon, upstream id parsed from a `url` file in firkin.files)` is deleted — once the item is in the user's collection, recommending it again is noise. Matching is done on `upstream_id` (not on the recommendation's CID) because the bookmarked firkin's CID typically diverges from the recommendation's virtual CID once artists / trailers have been resolved client-side.
...
- `PUT /api/recommendations/:firkin_id` — body `{ address, watched?, score? }`. Updates the user's `watched` flag and/or `score` (0..=100) on a single recommendation row. `404` when no row exists for the (address, firkin_id) pair; `400` for an invalid address or out-of-range score.
...
- `GET /api/catalog/:addon/:id/related` — returns `[{ id, title, year, description, posterUrl, backdropUrl, artists? }]` listing items related to the upstream catalog item. The base shape matches `/popular`; each entry additionally carries an `artists: [{ name, role?, imageUrl? }]` array (same shape as `/metadata`'s `artists`) so the WebUI's related grid can render the "artists & credits" column inline. For `tmdb-movie` / `tmdb-tv` this proxies TMDB's `/recommendations` endpoint, then fans out parallel `/credits` lookups per result via `tokio::task::JoinSet`; per-item credit failures degrade gracefully to an empty `artists` array on that item. For `musicbrainz` it browses other release-groups by the same primary artist (current release-group filtered out) — `artist-credit` is read straight off the browse response, so no extra HTTP calls are needed. Unknown / unsupported addons return an empty list. Used by `/catalog/virtual` and `/catalog/[ipfsHash]` to surface related items as virtual catalog links — the response is **not persisted** to SurrealDB and **not pinned** to IPFS; clicking a related item navigates to `/catalog/virtual?…` and only persists if the user explicitly bookmarks or picks a torrent there.
