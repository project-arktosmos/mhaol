...
- `POST /api/torrent/stream` — body `{ magnet }`. Wipes any previously-started stream torrents (deletes them from the session and removes their on-disk files in `<data_root>/downloads/torrent-streams/`), resolves the magnet metadata via librqbit's `list_only` flag (DHT + tracker peer discovery + BEP 9/10 metadata exchange — no piece downloads), picks the largest streamable video file (mp4/mkv/webm/mov/avi/m4v/ogv/ts), then starts a `only_files=[idx]` download into `torrent-streams/`. Returns `{ infoHash, name, fileIndex, fileName, fileSize, mimeType, streamUrl }`.
