...
- `track-resolver.svelte.ts` — `TrackResolver` class. Two entry points covering the two flows that need a tracklist: `loadFromFirkin({ releaseGroupId, files })` (used by the detail page) fetches the MusicBrainz tracklist and projects each track's YouTube URL + lyrics from the firkin's persisted `files` — *no JS-side searches* — returning `{ missingAny }` so the page knows whether to call `firkinsService.resolveTracks(id)` to fill in the gaps. `loadByReleaseGroup(...)` (used by the virtual catalog page, which has no firkin id yet) keeps the legacy in-browser search loop: per-track YouTube + LRCLIB queries with token-based scoring, exposing per-track status badges. Lyrics persisted on the firkin live as `'lyrics'`-typed `FileEntry` rows whose `value` is the JSON `{ source, externalId, syncedLyrics, plainLyrics, instrumental }`; the resolver decodes this on read and parses the LRC text into the existing `SubsLyricsItem` shape so `CatalogTracksCard` can show synced/plain/instrumental status without re-fetching.
