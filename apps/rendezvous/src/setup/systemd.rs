const SERVICE_NAME: &str = "mhaol-rendezvous";

pub fn install_service(config_path: &str) -> Result<(), String> {
    let exe_path = std::env::current_exe()
        .map(|p| p.to_string_lossy().to_string())
        .unwrap_or_else(|_| "/usr/local/bin/mhaol-rendezvous".into());

    let unit = format!(
        r#"[Unit]
Description=Mhaol Rendezvous (private-swarm IPFS bootstrap + WebRTC signaling)
