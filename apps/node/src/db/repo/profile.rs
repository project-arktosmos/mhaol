...
impl ProfileRepo {
    ...
    pub fn get_all(&self) -> Vec<ProfileRow> {
        ...
        let mut stmt = conn
            .prepare("SELECT username, wallet, added_at FROM profiles ORDER BY added_at DESC")
            .unwrap();
        stmt.query_map([], |row| {
            Ok(ProfileRow {
                username: row.get(0)?,
    ...
    pub fn get_by_wallet(&self, wallet: &str) -> Option<ProfileRow> {
        let conn = self.db.lock();
        conn.query_row(
            "SELECT username, wallet, added_at FROM profiles WHERE wallet = ?1",
            params![wallet],
            |row| {
                Ok(ProfileRow {
                    username: row.get(0)?,
    ...
    pub fn upsert(&self, username: &str, wallet: &str) {
        let conn = self.db.lock();
        conn.execute(
            "INSERT INTO profiles (wallet, username) VALUES (?1, ?2)
