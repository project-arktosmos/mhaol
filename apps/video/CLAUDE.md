...
These aliases are configured in both `svelte.config.js` and `vite.config.ts` and are **only valid inside `apps/video/`**:
...
Define shared types in `apps/video/src/types/`. One file per domain:
...
Tests live in `apps/video/test/` mirroring `src/`:
