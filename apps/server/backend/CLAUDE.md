...
```bash
# From repo root
pnpm dev:backend          # PORT=1530 cargo run -p mhaol-server --bin mhaol-server
pnpm build:backend        # cargo build --release --bin mhaol-server

# Tests
cargo test -p mhaol-server
cargo check -p mhaol-server
```
