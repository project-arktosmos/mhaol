...
const APPS: &[AppEntry] = &[
    ...
    AppEntry {
        name: "server",
        port: 1530,
        headless_script: "apps/server/scripts/headless.js",
    },
];
...
async fn check_health(
    ...
    // If we have a tracked status, use it for starting/failed
    let procs = state.hub.processes.lock().await;
    if let Some(proc) = procs.get(name.as_str()) {
        let tracked = proc.status.lock().await.clone();
        match tracked {
            AppStatus::Starting | AppStatus::Failed => {
...
async fn start_app(
    ...
    let logs = std::sync::Arc::new(Mutex::new(VecDeque::<String>::with_capacity(MAX_LOG_LINES)));
    ...
    // Spawn the headless script in the background with --skip-build
    // (all apps are pre-built when the hub starts)
    ...
    tokio::spawn(async move {
        {
            let msg = format!("Starting {}...", app_name);
        ...
        match cmd.spawn()
        {
            Ok(mut child) => {
                ...
                if let Some(stdout) = child.stdout.take() {
                    let logs_clone = std::sync::Arc::clone(&logs);
...
async fn get_logs(
    ...
    let logs = match procs.get(name.as_str()) {
        Some(proc) => {
            let buf = proc.logs.lock().await;
            buf.iter().cloned().collect()
        }
        None => vec![],
    };
    Json(LogsResult { name, logs })
}
