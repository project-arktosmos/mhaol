...
pub fn router() -> Router<AppState> {
    Router::new()
        ...
        .route("/content/favorites", get(list_favorites))
        .route("/content/fill-durations", post(fill_durations))
...
async fn list_channels(State(state): State<AppState>) -> impl IntoResponse {
    let rows = state.youtube_channels.get_all();
    let channels: Vec<YouTubeChannelJson> = rows
        .into_iter()
        .map(|r| YouTubeChannelJson {
            id: r.id,
            handle: r.handle,
            name: r.name,
            url: r.url,
...
// ===== YouTube Content CRUD =====

#[derive(Serialize)]
struct YouTubeContentJson {
...
fn map_content_row(row: crate::db::repo::youtube_content::YouTubeContentRow) -> YouTubeContentJson {
    YouTubeContentJson {
...
async fn list_content(State(state): State<AppState>) -> impl IntoResponse {
    let rows = state.youtube_content.get_all();
    Json(rows.into_iter().map(map_content_row).collect::<Vec<_>>())
}
...
async fn toggle_favorite(
    ...
    Json(serde_json::json!({ "isFavorite": is_favorite }))
}
...
async fn fill_durations(State(state): State<AppState>) -> impl IntoResponse {
    let missing = state.youtube_content.get_ids_missing_duration();
