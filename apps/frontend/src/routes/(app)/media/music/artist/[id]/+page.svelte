...
{#if catalogItem}
	<CatalogDetailPage
		...
		onback={() => goto(`${base}/media/music/artist`)}
				...
				<div>
					...
					<div class="grid grid-cols-2 gap-2 sm:grid-cols-3">
						{#each discography as album (album.id)}
							<CatalogCard card={catalogItemToCardData(album)} onclick={() => goto(`${base}/media/music/album/${album.sourceId}`)} />
						{/each}
					</div>
				</div>
			{/if}
...
{:else}
	<div class="flex flex-1 flex-col items-center justify-center gap-2">
		...
		<button class="btn btn-ghost btn-sm" onclick={() => goto(`${base}/media/music/artist`)}>Back to artists</button>
	</div>
{/if}
