...
<div class="p-4">
					...
					<div class="grid grid-cols-2 gap-3 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6">
						{#each pinnedItems as item (item.id)}
							<CatalogCard
								...
								onclick={() => goto(`${base}/media/music/${item.kind === 'album' ? 'album' : 'artist'}/${item.sourceId}`)}
							/>
						{/each}
					</div>
				{/if}
		...
		<section class="mb-8">
			<div class="mb-3 flex items-center justify-between">
				...
				<a href="{base}/media/music/album" class="btn btn-ghost btn-sm">View all</a>
			</div>
			{#if albumsLoading}
		...
		<section>
			<div class="mb-3 flex items-center justify-between">
				...
				<a href="{base}/media/music/artist" class="btn btn-ghost btn-sm">View all</a>
			</div>
			{#if artistsLoading}
				<div class="flex items-center justify-center py-12">
					<span class="loading loading-spinner loading-md"></span>
				...
				<div class="grid grid-cols-2 gap-3 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6">
					{#each artists as artist (artist.id)}
						<CatalogCard
							...
							onclick={() => goto(`${base}/media/music/artist/${artist.sourceId}`)}
						/>
					{/each}
				</div>
			{/if}
