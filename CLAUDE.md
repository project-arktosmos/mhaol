...
- `apps/cloud/CLAUDE.md` — Tray-only desktop Tauri shell (`mhaol-cloud-shell`, "Mhaol Cloud"). Embeds `mhaol-backend` and spawns `mhaol_backend::run()` from the Tauri setup hook so the release bundle is self-contained.
...
- **`apps/cloud/`** — Tauri shell (`mhaol-cloud-shell`, productName "Mhaol Cloud"). Tray-only wrapper that **embeds the backend** — the Tauri `setup` hook spawns `mhaol_backend::run()` on Tauri's tokio runtime, so the release `.app` / `.msi` / `.deb` / `.AppImage` is fully self-contained (no separate `mhaol-cloud` bin needed at runtime). The backend binds `0.0.0.0:9898`, serves the embedded SPA via `rust-embed`, and the tray's **Open** item launches that URL in the system browser. In dev (`pnpm dev`) the standalone bin still owns 9899 and Vite owns 9898 for HMR; the embedded copy is dormant during the dev strand.
...
- **Health** (modal, summoned from `/catalog`) — polls `/api/cloud/status` every 5 seconds while the modal is open and renders status, latency, uptime, bind, package health, and identities. Polling is paused when the modal is closed. The root route (`/`) is intentionally empty.
