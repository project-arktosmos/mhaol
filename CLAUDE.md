...
- `track-resolver.svelte.ts` — `TrackResolver` class. Holds `$state` for `tracks`, `status`, `playingIndex`, `playError`. The detail page calls `loadFromFirkin({ releaseGroupId, files })` which fetches the MB tracklist and projects YouTube URLs + lyrics from the firkin's persisted `files` (no JS search) — when any track is missing data, the page calls `firkinsService.resolveTracks(id)` to drive the server-side `POST /api/firkins/:id/resolve-tracks` endpoint which fetches and bakes both YT URLs and lyrics in one rollforward. The virtual catalog page (no firkin id yet) keeps `loadByReleaseGroup(...)` which runs the in-browser per-track YouTube + LRCLIB searches.
