...
- **Feed** (`/feed`) — single-card view onto the user's recommendations. Fetches `/api/recommendations?excludeActioned=true` so any item the user has previously liked, discarded, or bookmarked from this page is filtered out, sorts by `count DESC` then average review rating (`score / maxScore` averaged across all reviews) DESC, and renders the top entry through the shared `FirkinCard` component (recommendation row mapped to a minimal `CloudFirkin` shape — poster as the cover image, reviews surfaced as the existing rating badges). Three buttons: **Discard** records a negative signal, **Like** records a positive signal, **Bookmark** mints a real firkin via the same `firkinsService.create` flow as the recommendations table. After every action, the next card slides in (no navigation away, even on bookmark). Actions are persisted via `POST /api/recommendations/action` (one row per user × recommendation in the `recommendation_action` table, upserted in place so a later action overwrites an earlier one).
- **Catalog** (`/catalog`) — pick an addon (each addon owns a single content kind: e.g. `tmdb-movie`, `tmdb-tv`, `musicbrainz`, `youtube-video`), optionally narrow by genre, and browse popular items. The Rust server proxies upstream calls via `/api/catalog/*` so addon API keys (`TMDB_API_KEY`) stay server-side. Clicking "View details →" navigates to a `/catalog/visit?...` resolver route whose `+page.ts` POSTs `/api/firkins` with `bookmarked: false` and `redirect(303, …)`s to `/catalog/<returnedId>` — the same `/catalog/[id]` detail page used for full library items. The browse-cache firkin is real (one row per upstream item, dedup by content-address) but its `bookmarked` flag is `false`, so the detail page renders only the **Bookmark** action and skips identity / version history / files / torrent search / IPFS-Torrent tabs. Clicking **Bookmark** promotes the same record in place via `PUT /api/firkins/:id` with `{ bookmarked: true }` — no new firkin, no CID roll (the flag is not part of the firkin body), and the page reactively gains the full bookmarked surface (torrent search auto-fires, identity/version cards appear, etc.). Picking a torrent assigns it to the firkin and the torrent-completion background task takes over to pin the resulting files to IPFS and roll the firkin version forward.
