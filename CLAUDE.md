...
- **Feed** (`/feed`) — single-card view onto the user's recommendations alongside an "Up next" table of the next ~20 queued items. Fetches `/api/recommendations?excludeActioned=true` so any item the user has previously liked, discarded, or bookmarked from this page is filtered out, sorts by `count DESC` then `userRating DESC` (treating `None` as 0) then average review rating (`score / maxScore` averaged across all reviews) DESC, and renders the top entry through the shared `FirkinCard` component. A 5-star rating widget above the action row writes back to the recommendation via `POST /api/recommendations/rating` (each star = 20, so 1–5 stars maps to a 20–100 score on the recommendation's `userRating` field); rating doesn't advance the card, just records a value the listing's secondary sort can use. Three action buttons: **Discard** records a negative signal, **Like** records a positive signal, **Bookmark** mints a real firkin via the same `firkinsService.create` flow as the recommendations table. After every action, the next card slides in (no navigation away, even on bookmark). Actions are persisted via `POST /api/recommendations/action` (one row per user × recommendation in the `recommendation_action` table, upserted in place so a later action overwrites an earlier one).
