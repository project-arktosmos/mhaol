...
- `POST /api/firkins/:id/subtitle` — download a picked OpenSubtitles entry, convert SRT → VTT (the in-page player parses VTT only), pin the bytes to the embedded IPFS node via `IpfsManager::add_bytes`, record a `text/vtt` pin under the synthetic path `firkin://<id>/subtitle/<lang>/<externalId>`, and roll a `subtitle`-typed FileEntry onto the firkin's `files` array. Body mirrors the relevant fields of a `SubsLyrics` row: `{ source, externalId, url, language, display?, release?, format?, isHearingImpaired? }`. The persisted `value` is a JSON-encoded `{ source, externalId, url, cid, language, display?, release?, format: "vtt", isHearingImpaired }` blob — `cid` is what the player resolves through `/api/ipfs/pins/<cid>/file`. Re-attaching the same `(language, externalId)` pair is a no-op rollforward (the value is identical). cfg(not(target_os = "android")). 1MB hard cap on the upstream payload.
