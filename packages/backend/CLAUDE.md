...
├── state.rs             # CloudState: { db, identity_manager, ytdl_manager, torrent_manager, ipfs_manager, ipfs_stream_manager, track_progress, album_download_progress, ytdl_channel_cache, firkin_locks }. `firkin_locks` is a per-firkin async lock map (`Arc<parking_lot::Mutex<HashMap<String, Arc<tokio::sync::Mutex<()>>>>>`) — every code path that does a read-modify-write on a firkin (PUT, POST :id/files, POST :id/subtitle, POST :id/enrich, POST :id/resolve-tracks, POST :id/roms, POST :id/finalize, the torrent-completion watcher, the album-download task, the TV-build task) **must** acquire `state.firkin_lock(id).lock_owned()` before reading the current record. Without this serialisation, concurrent mutations would each load the same starting `files` array, mutate locally, and write back — the later writer would silently clobber the earlier one, exactly how a freshly-attached `torrent magnet` got dropped by the trailer resolver's `youtube preferred client` write.
...
├── firkins.rs           # /api/firkins CRUD — SurrealDB-backed firkin records (stable UUID id, content-addressed `cid` field); create also pins the body JSON to the embedded IPFS node and records an `ipfs_pin` row keyed `firkin://<id>`. Firkins reference artists by CID (see `artists.rs`); incoming creates speak in inline artist objects which the server upserts before computing the firkin CID. Carries a `bookmarked` flag (default `true`, **not** part of the CID body) — the `/catalog/visit` resolver creates non-bookmarked browse-cache rows; the listing endpoint defaults to `bookmarked === true` only. Every mutation handler (PUT, POST :id/files, POST :id/subtitle, POST :id/enrich, POST :id/finalize, POST :id/roms, POST :id/resolve-tracks) takes `state.firkin_lock(id).lock_owned()` for the entire read-modify-write so concurrent writers serialise; without that lock the legacy PUT path used to drop concurrent writes wholesale (e.g. a freshly-attached `torrent magnet` got clobbered by the trailer resolver's `youtube preferred client` write). Granular files mutations should always go through `POST /api/firkins/:id/files` rather than `PUT` with a `files` field — the granular endpoint reads the current state under the lock so the merge is atomic; PUT replaces from the client's snapshot.
├── album_download.rs    # `POST /api/firkins/:id/download-album` background per-track downloader for `musicbrainz` firkins. Walks the MusicBrainz tracklist, queues an audio-only yt-dlp download per persisted YouTube URL, pins each completed file to IPFS, and rolls the firkin forward with one `ipfs`-typed FileEntry per track (incremental rollforwards so the WebUI sees local files appear as each track finishes). Live state on `state.album_download_progress`. cfg(not(target_os = "android"))
...
- `TMDB_API_KEY` — Required for any `/api/catalog/tmdb-*` route (popular, search, genres, metadata, related). Without it those endpoints return `503`.
- `OMDB_API_KEY` — Optional. When set, `tmdb_metadata` (used by `GET /api/catalog/tmdb-{movie,tv}/:id/metadata`) makes a best-effort follow-up call to `omdbapi.com` keyed by the IMDb id and merges Rotten Tomatoes / Metacritic / IMDb entries into the returned `reviews` array. Failures (missing key, OMDb 404, transient outage) are swallowed silently — the TMDB review still surfaces. Free tier: 1000 req/day.
...
- `PUT /api/firkins/:id` — update any subset of `title`, `addon`, `description`, `artists`, `images`, `files`, `year`, `trailers`, `reviews`, `bookmarked`. Applies the mutation through `rollforward_firkin`: when the new body produces a different CID, the prior CID is pushed onto `version_hashes`, `version` is incremented, the record is updated in place at its stable UUID id, and the new body JSON is re-pinned to IPFS. The `bookmarked` field is **not** part of `serialize_firkin_payload` / `compute_firkin_cid`, so flipping it in isolation persists without rolling the version. A false→true bookmark flip on a `musicbrainz` firkin spawns `spawn_resolve_album_tracks` as a background task. **For `files` mutations, prefer `POST /api/firkins/:id/files`** — the PUT path replaces `files` wholesale from the *client's* snapshot, so concurrent callers (e.g. the trailer resolver caching its preferred YouTube client while the user picks a torrent) reliably drop one of the writes; the granular endpoint reads + mutates + writes under the per-firkin lock so the merge is always against the latest committed state.
- `POST /api/firkins/:id/files` — granular files mutation. Body: `{ add?: FileEntry[], removeTypes?: string[], removeEntries?: { type, value }[] }`. The handler holds `state.firkin_lock(id)` (per-firkin async lock, see [`state::FirkinLockMap`]) for the entire read-modify-write, drops every entry whose `kind` is in `removeTypes` or whose `(kind, value)` matches `removeEntries`, then appends `add`. Use `removeTypes: ["youtube preferred client"]` + a single `add` for the "replace the singular X" pattern. Use a bare `add` for "append a torrent magnet / subtitle / track url" without touching anything else. **This is the canonical fix for the "torrent magnet vanished from the firkin" bug** — `PUT` with a `files` field reliably loses concurrent writes; this endpoint never can. All adds are validated against `ALLOWED_FILE_TYPES` like the PUT/POST paths.
...
- `POST /api/firkins/:id/download-album` — kick the background album-download task for a `musicbrainz` firkin. Spawns `album_download::spawn_download_album` (fire-and-forget `tokio::spawn`), which walks the MusicBrainz tracklist, queues an audio-only yt-dlp download per track using the persisted YouTube URL, pins each completed file to IPFS via `IpfsManager::add` + `ipfs_pins::record_pin`, and rolls the firkin forward with a new `ipfs`-typed `FileEntry` per track (one rollforward per completed track so the WebUI sees local files appear incrementally). Idempotent: tracks that already have an `ipfs` file are skipped. Concurrent kicks while a task is in flight are no-ops. Returns 202 with the seeded progress payload.
- `GET /api/firkins/:id/download-progress` — live per-track download progress. Backed by `state.album_download_progress` (in-memory `Arc<RwLock<HashMap<firkin_id, AlbumDownloadProgress>>>` defined in `src/album_download.rs`). Returns 404 when no task has been spawned for this firkin.
...
- `POST /api/firkins/:id/subtitle` — download a picked OpenSubtitles entry, convert SRT → VTT (the in-page player parses VTT only), pin the bytes to the embedded IPFS node via `IpfsManager::add_bytes`, write a copy of the VTT to `<data_root>/downloads/subtitles/<cid>.vtt`, record a `text/vtt` pin row pointing at that on-disk path, and roll a `subtitle`-typed FileEntry onto the firkin's `files` array. Body mirrors the relevant fields of a `SubsLyrics` row: `{ source, externalId, url, language, display?, release?, format?, isHearingImpaired?, season?, episode? }`. `season` / `episode` are TV-only — `tmdb-tv` firkins pin the subtitle to the currently-playing episode so a single firkin can carry one row per `(season, episode, language, externalId)` without dedup colliding. Movies leave both unset. The persisted `value` is a JSON-encoded `{ source, externalId, url, cid, language, display?, release?, format: "vtt", isHearingImpaired, season?, episode? }` blob — `cid` is what the player resolves through `/api/ipfs/pins/<cid>/file`. Re-attaching the same `(language, externalId, season, episode)` tuple is a no-op rollforward (the value is identical). cfg(not(target_os = "android")). 1MB hard cap on the upstream payload.
- **Firkin `reviews`**: firkin records carry an optional `reviews: [{ label, score, maxScore, voteCount? }]` array — TMDB contributes `{ label: "TMDB", score: vote_average, maxScore: 10, voteCount: vote_count }`, MusicBrainz contributes `{ label: "MusicBrainz", score: rating.value, maxScore: 5, voteCount: rating.votes-count }`, and (when `OMDB_API_KEY` is set and the upstream item resolves to an IMDb id) OMDb contributes `{ label: "Rotten Tomatoes", score, maxScore: 100 }`, `{ label: "Metacritic", score, maxScore: 100 }`, and `{ label: "IMDb", score, maxScore: 10, voteCount: imdbVotes }`. All are extracted server-side by `GET /api/catalog/:addon/:id/metadata`. Reviews with zero votes (TMDB / MusicBrainz) or unparseable values (`"N/A"` from OMDb) are filtered out. Merge is by `label` — duplicate labels are dropped in favour of the entry already present, and any user-provided reviews carrying their own labels (added via `PUT /api/firkins/:id`) are preserved alongside the upstream entries. The field participates in `compute_firkin_cid`. **Server-side OMDb backfill**: every firkin write path runs `maybe_backfill_omdb_reviews` (in `firkins.rs`) — `POST /api/firkins` calls it directly, and `rollforward_firkin` calls it on every mutation that flows through the rollforward pipeline (`PUT /api/firkins/:id`, `POST /api/firkins/:id/{files,enrich,subtitle,roms,finalize,resolve-tracks}`, the torrent-completion / tv-build / album-download background tasks). The helper is gated on the firkin being `tmdb-movie` / `tmdb-tv` and missing every OMDb-sourced label (`Rotten Tomatoes` / `Metacritic` / `IMDb`), so it's a free no-op once enrichment has succeeded once but it transparently fixes up firkins minted before `OMDB_API_KEY` was set (or those whose initial enrichment transiently failed) at the next mutation, without depending on the catalog detail page's frontend backfill effect to run.
...
- `GET /api/torrent/list`, `POST /api/torrent/add`, `POST /api/torrent/evaluate`, `POST /api/torrent/stream`, `GET /api/torrent/stream/:info_hash/:file_index` — torrent client surface backed by `librqbit`. `evaluate` caches its outcome per `info_hash` in the `torrent_eval` table (streamability is intrinsic to the SHA1 of the info dict, so the cache never expires).
- `GET /api/firkins/:id/torrent-search` — return the persisted torrent search results for a firkin (table `firkin_torrent_search`, keyed by firkin id). Response body: `{ firkinId, addon, query, category, results: TorrentResult[], evals: { [info_hash]: EvaluateResponse }, createdAt, updatedAt }` or JSON `null` when no row exists. The `evals` map is joined from `torrent_eval` so the WebUI can rehydrate the streamability column without replaying every `/evaluate` round-trip.
- `POST /api/firkins/:id/torrent-search` — body `{ addon, query, category? }`. Runs a fresh PirateBay query via `crate::search::run_torrent_query`, persists the results to `firkin_torrent_search`, and returns the same shape as the GET. Used by the WebUI's **Refresh** button (and by the initial-search code path on first visit when no cache row exists).
