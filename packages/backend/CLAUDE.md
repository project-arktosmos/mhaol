...
- `GET /api/recommendations?address=<>&excludeActioned=<bool>` — list rows from the `recommendation` table for the given user. Sort: `count DESC, mean(score / maxScore across reviews) DESC, updated_at DESC`. Pass `excludeActioned=true` to drop any row the user has acted on (`recommendation_action` table — see `POST /action`); the `/feed` page uses this to avoid showing the same card twice.
