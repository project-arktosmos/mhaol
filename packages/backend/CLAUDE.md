...
├── library_scan.rs      # Library scan side-effects: lite-pin every matched file (compute its UnixFS CID via `IpfsManager::compute_file_cid`, no `put_block` writes). Materialisation is deferred to `/api/ipfs/pins/:cid/materialise` so a 200 GB TV library doesn't duplicate every byte into the blockstore on scan. No firkin creation. cfg(not(target_os = "android"))
...
**Materialisation** (the actual `repo.put_block` writes that make the CID reachable to other peers via bitswap) happens lazily through `POST /api/ipfs/pins/:cid/materialise`. Until that endpoint is hit, the CID is a "local-only" reference: the cloud knows the file's content address, but other clouds can't bitswap-fetch its blocks. This is intentional — the user opts in to swarm sharing per-CID instead of blanket-duplicating the entire library at scan time.
