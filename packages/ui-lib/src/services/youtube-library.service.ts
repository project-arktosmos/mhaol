...
class YouTubeLibraryService {
	...
	subtitleUrl(youtubeId: string, lang: string): string {
		return apiUrl(`/api/youtube/content/${youtubeId}/subtitles/${lang}`);
	}
	...
	async listSubtitles(
		...
		try {
			return await this.fetchJson(`/api/youtube/content/${youtubeId}/subtitles`);
		} catch {
			return [];
		}
	}
	...
	> {
		return this.fetchJson(`/api/youtube/content/${youtubeId}/subtitles`, {
			method: 'POST',
	...
	async deleteAudio(youtubeId: string): Promise<void> {
		const response = await fetch(apiUrl(`/api/youtube/content/${youtubeId}/audio`), {
			method: 'DELETE'
		});
	...
	async deleteVideo(youtubeId: string): Promise<void> {
		const response = await fetch(apiUrl(`/api/youtube/content/${youtubeId}/video`), {
			method: 'DELETE'
		});
		if (!response.ok) throw new Error(`HTTP ${response.status}`);
		await this.fetchContent();
