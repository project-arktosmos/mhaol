<script lang="ts">
	...
	type MediaSource =
		| { type: 'video'; src: string; mimeType?: string; subtitles?: SubtitleTrackInfo[] }
	...
	// ===== Subtitle Overlay (custom renderer — bypasses flaky <track> element) =====
	...
	// Fetch and parse VTT when subtitles change
	$effect(() => {
		const subs = source.type === 'video' ? source.subtitles : undefined;
		subCues = [];
		activeSubText = '';
		if (!subs?.length) return;

		const url = subs[0].url;
		fetch(url)
			.then((r) => (r.ok ? r.text() : ''))
			.then((vtt) => {
				if (vtt) subCues = parseVtt(vtt);
			})
			.catch(() => {});
	});

	// Update active subtitle text based on current playback time
	...
	$effect(() => {
		// Re-run whenever currentTime changes
		currentTime;
		updateSubtitleText();
	});
</script>
	...
	<div data-media-player class="flex flex-col gap-0 overflow-hidden rounded-lg bg-black">
		{#if source.type === 'video'}
			<div class="relative">
				...
				{#if activeSubText}
					<div class="pointer-events-none absolute inset-x-0 bottom-8 flex justify-center px-4">
						<span
							class="inline-block max-w-full rounded bg-black/80 px-3 py-1 text-center text-sm leading-snug text-white"
						>
							{activeSubText}
						</span>
					</div>
