<script lang="ts">
	...
	async function loadPlugins() {
		...
		error = null;
		try {
			const res = await fetchRaw('/api/plugins');
	...
	async function saveSetting(pluginName: string, key: string) {
		saving = true;
		try {
			const res = await fetchRaw('/api/plugins/settings', {
				method: 'PUT',
...
<div class="pr-8">
	...
	<p class="text-sm text-base-content/60">Manage installed plugins, processes, and settings</p>
</div>
