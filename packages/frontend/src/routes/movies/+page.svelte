<script lang="ts">
	...
	import type { DisplayTMDBMovie } from 'tmdb/types';
	import { moviesToDisplay } from 'tmdb/transform';
	...
	async function loadMovies(category: DiscoverCategory, p: number = 1) {
		...
		try {
			const res = await fetch(`/api/tmdb/movies?category=${category}&page=${p}`);
			const data = await res.json();
			if (!res.ok) {
				error = data.error ?? 'Failed to load movies';
				return;
			}
			movies = moviesToDisplay(data.results);
			page = data.page;
	...
	async function searchMovies(p: number = 1) {
		...
		try {
			...
			const res = await fetch(`/api/tmdb/search/movies?${params}`);
			const data = await res.json();
			if (!res.ok) {
