...
<div class="flex flex-col gap-3">
	...
	<div class="flex items-center gap-2">
		...
		<button class="btn btn-primary btn-sm" onclick={handleSelectCurrent}>
			Select This Directory
		</button>
	</div>

	<!-- Current path display -->
	<div class="rounded-lg bg-base-300 px-3 py-2 text-sm font-mono">
		...
		{:else}
			{#each $state.browseDirectories as entry (entry.path)}
				<div class="flex items-center border-b border-base-200 last:border-b-0">
					<button
						class="btn btn-ghost btn-sm flex-1 justify-start gap-2 rounded-none font-normal"
						onclick={() => handleNavigate(entry.path)}
					>
						<svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 text-warning" fill="currentColor" viewBox="0 0 24 24">
