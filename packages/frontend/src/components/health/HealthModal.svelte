<script lang="ts">
	...
	const modalStore = healthModalService.store;
...
<Modal open={$modalStore.open} maxWidth="max-w-7xl" onclose={close}>
	<div class="flex flex-col gap-6">
		<header class="flex items-center justify-between gap-4">
			<div>
				<h2 class="text-2xl font-bold">Cloud Node Health</h2>
		...
		{#if $state.status?.client_address}
			<section class="card bg-base-200 p-4">
				<h3 class="mb-2 text-lg font-semibold">Identities</h3>
