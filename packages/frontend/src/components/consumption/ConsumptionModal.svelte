<script lang="ts">
	...
	const modalStore = consumptionModalService.store;
...
<Modal open={$modalStore.open} maxWidth="max-w-6xl" onclose={close}>
	<div class="flex flex-col gap-6">
		<header class="flex flex-col gap-1">
			<h2 class="text-2xl font-semibold">Consumption</h2>
			<p class="text-sm text-base-content/60">
				Playback time per firkin, accumulated by the right-side player. The tracker writes a
				heartbeat every 10 seconds while a firkin file is streaming and not paused; pauses,
				buffering, and connection teardown stop time accruing. Music firkins expand to show
				per-track listening time.
			</p>
		</header>
								...
								{:else if trackers.length === 0}
									<tr>
										<td colspan="5" class="text-center text-base-content/60">
											No consumption recorded yet — start playing a firkin from the right-side
											player to see it here.
										</td>
									</tr>
								{:else}
